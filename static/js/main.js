...
function goToSlide(i) {
  ...
  // Calculate pixel offset relative to the carousel viewport (parent element)
  // derive slide width from first slide DOM rect (handles padding/scrollbar accurately)
  const firstSlide = carouselState.slides[0];
  const slideWidth = firstSlide ? firstSlide.getBoundingClientRect().width : carouselState.container.clientWidth;
  const offsetPx = -i * slideWidth;
  carouselState.container.style.transform = 'translateX(' + offsetPx + 'px)';
...
window.toggleMenu = function () {
  ...
  if (menu) {
    // toggle both class names in case CSS uses either
    menu.classList.toggle('open');
    menu.classList.toggle('show');
    const expanded = menu.classList.contains('open') || menu.classList.contains('show');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (container) {
    ...
    // Ensure slides occupy the carousel viewport exactly (use pixels to avoid percent mismatches)
    const carouselEl = container.closest('.carousel');
    function layoutSlides() {
      const viewportWidth = carouselEl ? Math.floor(carouselEl.getBoundingClientRect().width) : container.clientWidth;
      // set each slide width to viewportWidth in px
      ...
      container.style.width = (viewportWidth * carouselState.slides.length) + 'px';
      // reapply transform for current index
      const offsetPx = -carouselState.index * viewportWidth;
    ...
    // Recalculate slide sizes on resize
  ...
  // Attach a robust click listener to the menu toggle (in case inline onclick isn't firing)
